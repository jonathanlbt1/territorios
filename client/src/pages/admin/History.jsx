...
function AdminHistory() {
  ...
  const filteredHistory = history.filter(h =>
    ...
    h.dirigente_name?.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-display font-bold text-slate-800 dark:text-white">Histórico</h1>
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          {history.length} registros de trabalhos concluídos
        </p>
      </div>
      ...
      <div className="card p-4">
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex-1 min-w-[200px]">
            <label htmlFor="start-date-filter" className="input-label text-xs">Data Inicial</label>
            <input
      ...
      {filteredHistory.length === 0 ? (
        <EmptyState
          ...
          description="O histórico aparecerá aqui após validar devoluções"
        />
      ) : (
        <div className="space-y-3">
          {filteredHistory.map(item => (
            <div key={item.id} className="card overflow-hidden">
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  <div className="flex items-center gap-4 mt-1 text-xs text-slate-400">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {getValidatedDate(item.validated_at)
                        ? format(getValidatedDate(item.validated_at), "dd/MM/yyyy", { locale: ptBR })
                        : 'Data indisponível'}
                    </span>
                  </div>
                </div>
              ...
              {/* Expanded Details */}
              {expandedId === item.id && (
                <div className="px-4 pb-4 pt-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      <p className="text-slate-500 dark:text-slate-400">Validado por</p>
                      <p className="font-medium text-slate-800 dark:text-white">{item.validated_by_name || 'N/A'}</p>
                    </div>
                    {item.return_observations && (
                      <div className="col-span-2">
                        <p className="text-slate-500 dark:text-slate-400">Observações do Dirigente</p>
